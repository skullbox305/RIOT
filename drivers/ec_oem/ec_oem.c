...
int ec_oem_init(ec_oem_t *dev, const ec_oem_params_t *params)
{
    ...
    /* Register read test */
    if (i2c_read_regs(I2C, ADDR, EC_OEM_REG_DEVICE_TYPE, &reg_data, 1,
                      0x0) < 0) {
                                  ...
                                  uint32_t calibration_value)
{
    ...
    printf("calibration value %ld:", calibration_value);
    ...
    if (i2c_write_regs(I2C, ADDR, EC_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0)
        < 0) {
    ...
    if (i2c_read_regs(I2C, ADDR, EC_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0)
        < 0) {
    ...
    uint32_t confirm_value = (int32_t)(reg_value[0] << 24) |
                             (int32_t)(reg_value[1] << 16)
                             | (int32_t)(reg_value[2] << 8) |
                             (int32_t)(reg_value[3]);

    if (confirm_value != calibration_value) {
        DEBUG("\n[ec_oem debug] Setting calibration register to EC raw %ld "
              "failed \n", calibration_value);
        i2c_release(I2C);
        return EC_OEM_WRITE_ERR;
    }
