...
int rtd_oem_init(rtd_oem_t *dev, const rtd_oem_params_t *params)
{
	...
	/* Register read test */
	if (i2c_read_regs(I2C, ADDR, RTD_OEM_REG_DEVICE_TYPE, &reg_data, 1, 0x0)
			< 0)
	{
		...
		uint32_t calibration_value)
{
	...
	if (i2c_write_regs(I2C, ADDR, RTD_OEM_REG_CALIBRATION_BASE, &reg_value, 4,
			0) < 0)
	{
	...
	if (i2c_read_regs(I2C, ADDR, RTD_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0)
			< 0)
	{
		...
		rtd_oem_calibration_option_t option)
{
	...
	if (i2c_write_reg(I2C, ADDR, RTD_OEM_REG_CALIBRATION_REQUEST, option, 0)
			< 0)
	{
...
int rtd_oem_read_temp(const rtd_oem_t *dev, int32_t *rtd_value)
{
	...
	if (i2c_read_regs(I2C, ADDR, RTD_OEM_REG_RTD_READING_BASE, &reg_value, 4, 0)
			< 0)
	{
	...
	*rtd_value = (int32_t)((int32_t) (reg_value[0] < 24)|(int32_t) (reg_value[1] << 16) | (int32_t) (reg_value[2] << 8)
			| (int32_t) (reg_value[3]));
